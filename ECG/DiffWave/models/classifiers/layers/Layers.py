...
class UpsamplingConvo1D(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, upsample, kernel_size, stride, padding
    ):
...
class WaveGANDeconvolution(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size, stride, padding, upsampling=False
    ):
        super(WaveGANDeconvolution, self).__init__()
        if upsampling:
            self.deconvo = UpsamplingConvo1D(
                in_channels, out_channels, stride, kernel_size, 1, 0
            )
        else:
            self.deconvo = nn.ConvTranspose1d(
...
class Patchify(nn.Module):
    ...
    def __init__(self, in_channels, embed_dimension, patch_size, norm_layer=True):
...
class PatchDiscriminator1D(nn.Module):
    ...
    def __init__(
        ...
        self.output = nn.Sequential(
            nn.Conv1d(
                embed_dimension, sequence_length // patch_size, kernel_size=1, stride=1
            ),
            nn.Sigmoid(),
        )
