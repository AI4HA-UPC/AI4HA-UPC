...
class UNet1DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if use_timestep_embedding:
            ...
            time_embed_dim = self.sample_size
    ...
    def forward(
        ...
        # Time step embedding adds an additional projection to the intial timestep embedding
        if self.config.use_timestep_embedding:
            ...
            if self.class_embedding is not None:
                ...
                if self.config.class_embed_type == "timestep":
                    ...
                    class_emb = self.class_embedding(
                        class_labels.to(dtype=torch.float32)).to(
                            dtype=self.dtype)

                elif self.config.class_embed_type == "identity":
                    class_emb = self.class_embedding(
                        class_labels.to(dtype=torch.float32)).to(
                            dtype=self.dtype)
                    class_emb = class_emb.repeat(
                        [1, timestep_embed.shape[1]]).reshape(
                            (sample.shape[0],
                             timestep_embed.shape[1])).to(sample.dtype)
                timestep_embed = (timestep_embed +
                                  class_emb).to(dtype=torch.float32)

        else:
            # No additional projection means repeating the timestep embedding to match the sample size
            # The dimension of the timestep embedding is the same as the sample size
            ...
            if self.class_embedding is not None:
                ...
                if self.config.class_embed_type == "timestep":
                    # projects to the time embedding and adds the class embedding
                    class_labels = self.time_proj(class_labels)
                    class_emb = self.class_embedding(
                        class_labels.to(dtype=torch.float32)).to(
                            dtype=self.dtype)
                    ...
                    # indentity projection dies nothing, we just repeat the class embedding to match the sample size
                    class_emb = self.class_embedding(
                        class_labels.to(dtype=torch.float32)).to(
                            dtype=self.dtype)
                    class_emb = class_emb.repeat([1, sample.shape[2]]).reshape(
                        (sample.shape[0], 1, sample.shape[2])).to(sample.dtype)
