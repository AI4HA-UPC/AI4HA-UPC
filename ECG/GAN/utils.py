...
def split_data(audio_path_list, valid_ratio, test_ratio, batch_size):
    ...
    if not (num_valid > 0 and num_test > 0 and num_train > 0):
        LOGGER.error(
            "Please download DATASET '{}' and put it under current path !".format(
                DATASET_NAME
            )
        )
...
def calc_gradient_penalty(
    net_dis, real_data, fake_data, batch_size, lmbda, use_cuda=False
):
...
# Adapted from https://github.com/caogang/wgan-gp/blob/master/gan_toy.py
def calc_gradient_penalty_with_gt(
    net_dis, real_data, fake_data, batch_size, lmbda, use_cuda=False, gt_code=None
):
    # Compute interpolation factors
...
# Adapted from https://github.com/caogang/wgan-gp/blob/master/gan_toy.py
def calc_gradient_penalty_with_gt_v2(
    net_dis,
    real_data,
    fake_data,
    batch_size,
...
def plot_loss(
    D_cost_train, D_wass_train, D_cost_valid, D_wass_valid, G_cost, save_path
):
    assert (
        len(D_cost_train)
        == len(D_wass_train)
...
def get_plots(true_ecg, fake_ecg):
    ...
    for i in range(8):
        sns_plot = sns.lineplot(
            x=[i for i in range(len(true_ecg_8_chs[i]))], y=true_ecg_8_chs[i], ax=axs[0]
        )
        sns_plot = sns.lineplot(
            x=[i for i in range(len(fake_ecg_8_chs[i]))], y=fake_ecg_8_chs[i], ax=axs[1]
        )
...
def get_plots_all(true_ecgs, fake_ecgs, num_of_plots=4):
    ...
    for b in range(bs):
        ...
        for i in range(8):
            sns_plot = sns.lineplot(
                x=[i for i in range(len(true_ecg_8_chs[i]))],
                y=true_ecg_8_chs[i],
                ax=axs[b, 0],
            )
            sns_plot = sns.lineplot(
                x=[i for i in range(len(fake_ecg_8_chs[i]))],
                y=fake_ecg_8_chs[i],
                ax=axs[b, 1],
            )
