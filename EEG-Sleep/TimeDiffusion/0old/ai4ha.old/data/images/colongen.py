...
class ColonGenDataset(GenericSegmentationDataset):
    def __init__(self, trans='TR512', temp=1, topk=10, size=256,
        ...
        super(ColonGenDataset, self).__init__(common_transform, transform, mask_transform)

        if [trans, temp, topk, size] not in DATASETS:
            raise NameError("COLON: Unknown dataset")


        base_path = f'/home/bejar/bsc/Data/ColonGen/DD{size}/{trans}/S{temp}'
...
class ColonGenDataModule(pl.LightningDataModule):
    @staticmethod
    ...
    def __init__(self, batch_size, subdatasets, training_modality=TrainingSplitModality.TEST_ONLY,
                 ...
                 num_workers=128):
        super(ColonGenDataModule, self).__init__()
    ...
    def setup(self, stage: str) -> None:
        if not self.setup_done:
            datasets = [ColonGenDataset(*args) for args in self.subdatasets]
