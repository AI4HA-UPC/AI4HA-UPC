...
class SUNColonGenDataset(GenericSegmentationDataset):
    def __init__(self, trans='TR512', temp=1, topk=10, size=256,
        ...
        super(SUNColonGenDataset, self).__init__(common_transform, transform, mask_transform)

        if [trans, temp, topk, size] not in DATASETS:
            raise NameError("SUNCOLON: Unknown dataset")
...
class SUNColonGenDataModule(pl.LightningDataModule):
    @staticmethod
    ...
    def __init__(self, batch_size, subdatasets, training_modality=TrainingSplitModality.TEST_ONLY,
                 ...
                 num_workers=128):
        super(SUNColonGenDataModule, self).__init__()
    ...
    def setup(self, stage: str) -> None:
        if not self.setup_done:
            datasets = [SUNColonGenDataset(*args) for args in self.subdatasets]
