# -*- coding: UTF-8 -*-
# Main code extracted from https://github.com/birdx0810/timegan-pytorch
...
class RecoveryNetwork(torch.nn.Module):
    ...
    def forward(self, H, T, labels=None, prev_step=None):
        ...
        H = torch.cat((H, x, prev_step), dim=-1) if labels is not None and prev_step is not None else H 
...
class GeneratorNetwork(torch.nn.Module):
    ...
    def forward(self, Z, T, labels=None, prev_step=None):
        ...
        Z = torch.cat((Z, x, prev_step), dim=-1) if labels is not None and prev_step is not None else Z 
...
class TimeGAN(torch.nn.Module):
    ...
    def _discriminator_forward(self, X, T, Z, gamma=1, labels=None, prev_step=None):
        ...
        # Gradient penalty
        #gradient_penalty = self.compute_gradient_penalty(self.discriminator, H, H_hat, T, labels, prev_step)
        ...
        Y_class_true_avg = torch.mean(Y_class_real, dim=1)  
        labels = labels.to(Y_class_real.device)
        D_loss_cls_true = self.crit_cls(Y_class_true_avg, labels)

        Y_class_fake_avg = torch.mean(Y_class_fake, dim=1)  
    ...
    def _inference(self, Z, T, labels=None, prev_step=None):
        ...
        # Synthetic data generated
        # Recovery: Transforms the output of the supervisor (H_hat) back into the original data space, resulting in the synthetic data
