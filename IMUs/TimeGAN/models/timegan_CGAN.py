...
class TimeGAN(torch.nn.Module):
    ...
    def _recovery_forward(self, X, T, labels, prev_step=None): # The embedding and recovery 
        ...
        #X_tilde = self.recovery(H, T, labels, prev_step=prev_step)
    ...
    def _generator_forward(self, X, T, Z, gamma=1, labels=None, prev_step=None):
        ...
        """Y_fake = self.discriminator(H_hat, T, labels, prev_step=prev_step)  # Output of supervisor
        Y_fake_e = self.discriminator(E_hat, T, labels, prev_step=prev_step)"""  # Output of generator
