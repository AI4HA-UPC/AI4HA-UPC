...
def data_preprocess(
    ...
    for i in tqdm(range(no)):
        ...
        curr_data = curr_data[:, 1:] # Drop 0 column

        # Impute missing data
        curr_data = imputer(curr_data, impute_vals)

        # Normalize data
        #curr_data[:, 1:] = scaler.transform(curr_data[:, 1:])
        ...
        if curr_no >= max_seq_len:
            output[i, :, :] = curr_data[:max_seq_len, 1:]  # Shape: [1, max_seq_len, dim] # Drop Idx column
            time.append(max_seq_len)
        else:
            output[i, :curr_no, :] = curr_data[:, 1:]  # Shape: [1, max_seq_len, dim] # Drop Idx column
            #last_val = curr_data[-1, 1:]
            #output[i, curr_no:, :] = last_val  # Padding with the last value
            time.append(curr_no)
