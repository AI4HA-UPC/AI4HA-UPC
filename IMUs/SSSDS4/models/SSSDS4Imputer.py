...
class Residual_block(nn.Module):
    def __init__(self, res_channels, skip_channels, 
        ...
        self.cond_conv = Conv(3*in_channels + 1, 2*self.res_channels, kernel_size=1)  
...
class SSSDS4Imputer(nn.Module):
    ...
    def forward(self, input_data):
        ...
        noise, conditional, mask, diffusion_steps, label, prev_step  = input_data 

        conditional = conditional * mask
        conditional = torch.cat([conditional, mask.float()], dim=1)
        # Concatenate label and prev_step with conditional
        label = label.to(conditional.device)
        prev_step = prev_step.to(conditional.device)
        prev_step = prev_step.permute(0, 2, 1)
        sequence_length = conditional.size(2)
        label = label.unsqueeze(1).repeat(1, 1, sequence_length)
        label = label.permute(1, 0, 2)
        conditional = torch.cat([conditional, label, prev_step], dim=1)
