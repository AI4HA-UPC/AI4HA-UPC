...
def sampling(net, size, diffusion_hyperparams, cond, mask, label, prev_step, only_generate_missing=0, guidance_weight=0):
    """
    ...
    with torch.no_grad():
        for t in range(T - 1, -1, -1):
            ...
            epsilon_theta = net((x, cond, mask, diffusion_steps, label, prev_step))  # predict \epsilon according to \epsilon_\theta
...
def training_loss(net, loss_fn, X, label, prev_step, diffusion_hyperparams, only_generate_missing=1):
    """
    ...
    epsilon_theta = net(
        (transformed_X, cond, mask, diffusion_steps.view(B, 1), label, prev_step,))  # predict \epsilon according to \epsilon_\theta
