...
from models.unet_1d_originalAllDatasets import CondUNet1DModel
...
from pipelines.pipeline_ddpm_1d_allDatasets import DDPMPipeline
from preprocess.augmentation import augmentation_mix_up
from log.textlog import textlog

import pickle
from diffusers.utils.torch_utils import randn_tensor
from GaitDatasetChunked import GaitDatasetChunked
...
def main(config):
    ...
    yaml_directory = ["../../AnnotatedSeries/GaitDatabase", "../../AnnotatedSeries/PD-CSIC"]
    csv_directory = ["../../AnnotatedSeries/GaitDatabase", "../../AnnotatedSeries/PD-CSIC"]
    
    columns_to_extract = ['l_foot_x', 'l_foot_y', 'l_foot_z', 'r_foot_x', 'r_foot_y', 'r_foot_z']
    chunk_size = 1000
    
    dataset = GaitDatasetChunked(yaml_directory, csv_directory, columns_to_extract, chunk_size)
    train_dataloader = DataLoader(dataset, batch_size=64, shuffle=True, collate_fn=GaitDatasetChunked.custom_collate_fn)    
    ...
    for epoch in range(first_epoch, config['train']['epochs']):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            clean_images, labels_train = batch
            ...
            with accelerator.accumulate(model):
                ...
                # labels = None if not class_conditioned else labels.to(
                #     dtype=torch.float32).to(clean_images.device)
    ...
    if accelerator.is_main_process:
        ...
        for batch in tqdm(train_dataloader, desc="Collecting training data"):
            ...
            clean_images, batch_labels = batch
    ...
    if accelerator.is_main_process:
        ...
        #samples_labs = [sample.cpu().numpy() for sample in samples_labs]
