...
class ColonData(Dataset):
    def __init__(self,
        ...
        for dcsv, droot, sroot in zip(data_csv, data_root, segmentation_root):
            with open(dcsv, "r") as f:
               image_paths = f.read().splitlines()
            self._length = len(image_paths)
            self.images = self.images + [os.path.join(droot, l)
                            for l in image_paths]
            self.masks = self.masks + [os.path.join(sroot, l.replace(".jpg", ".txt"))
                                   for l in image_paths]
    ...
    def __getitem__(self, i):
        ...
        f = open('/gpfs/projects/bsc70/bsc70642/misiones/Imagenes/Modelos/DenoisingDiffusion/Diffusers/run/LOG.txt', 'w')
        ...
        elif '.txt' in mask_path:
            mask = load_bbox(mask_path, image.shape)
        else:
        ...
        onehot = np.eye(self.nlabels)[segmentation].astype(np.float32)
