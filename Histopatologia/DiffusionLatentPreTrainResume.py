...
def main(config):
    ...
    if 'conditioning_dropout' in config['train']:
        from diffusion.models.unet_2d import UNet2DModel
    else:
    ...
    model = UNet2DModel().from_pretrained(f"{BASE_DIR}/checkpoints/{path}/unet")
    ...
    if 'ema' in config:
        ...
        #)
        ema_model = EMAModel.from_pretrained(f"{BASE_DIR}/checkpoints/{path}/unet_ema", model_cls=UNet2DModel)
