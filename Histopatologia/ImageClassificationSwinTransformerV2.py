"""
Training from scratch a ViT model using Transformers package of Hugging Face
"""
import argparse
import json
import logging
...
import transformers
from transformers import AutoConfig, ViTImageProcessor, SwinForImageClassification, SchedulerType, get_scheduler
...
def main(config):
    ...
    if config['train']['pretrain']:
        ...
        if "hidden_dropout_prob" in config["model"]:
            ...
            model = SwinForImageClassification(config_model)
        else:
            model = SwinForImageClassification(config_model)
