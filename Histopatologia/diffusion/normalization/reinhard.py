...
import cv2 as cv
import numpy as np
import diffusion.normalization.stain_utils as ut
...
# Class for torchvision 
class NormalizerTransform:
    """
    A stain normalization object that can be integrated into torchvision transform pipelines.
    """
    
    def __init__(self):
        super(NormalizerTransform, self).__init__()
        self.target_means = None
        self.target_stds = None
    ...
    def __call__(self, I):
        ...
        norm2 = ((I2 - means[1]) * (self.target_stds[1] / stds[1])) + self.target_means[1]
        norm3 = ((I3 - means[2]) * (self.target_stds[2] / stds[2])) + self.target_means[2]
        normalized_image = merge_back(norm1, norm2, norm3)
