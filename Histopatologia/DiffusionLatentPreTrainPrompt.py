"""
Generation of images using conditional diffusion model where the conditioning is a prompt indicating the type of cancer and the gleason score. 
"""
import argparse
import inspect
import logging
...
from diffusers import DDPMScheduler, DDIMScheduler, DPMSolverMultistepScheduler, UNet2DConditionModel, PNDMScheduler
...
def main(config):
    ...
    for epoch in range(first_epoch, config['train']['epochs']):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # if start == 0:
            #     print_memory(logger, accelerator, "INNER LOOP - LATENT")
            #     start = 1
            ...
            with accelerator.accumulate(model):
                ...
                #prompts = [f"Generate a pair of image and segmentation mask with a Gleason score of {new_labels[label.item()]}" for label in labels]
                ...
                if config['diffuser']['prediction_type'] == "epsilon":
                    ...
                    snr_weights = 1 # We can use others 
                    loss = snr_weights * F.mse_loss(model_output, v_pred, reduction="none") 
                    loss = loss.mean()
                else:
