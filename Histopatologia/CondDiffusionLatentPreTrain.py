...
def main(config):
    ...
    desired_dataset_size = len(train_data) // 20
    ...
    reference_mask_latents = train_data[0]['segmentation'].unsqueeze(0)
    reference_mask_reduced_latents = train_data[0]['segmentation_reduced'].unsqueeze(0)

    reference_mask = AE.decode(reference_mask_latents.to('cuda'), return_dict=False)[0] 
    reference_mask = reference_mask.clamp(0, 1).cpu().numpy()
    reference_mask = np.round(reference_mask * 5).astype(np.uint8)
    reference_mask = torch.from_numpy(reference_mask)
    print(reference_mask.min(), reference_mask.max())
    grid = T.ToPILImage(mode='RGB')(make_grid(reference_mask/5, nrow=1))
    grid.save(f"{BASE_DIR}/samples/reference_mask.jpg")
    ...
    for epoch in range(first_epoch, config['train']['epochs']):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            mask_latents = batch['segmentation_reduced']
            mask_embedding = mask_latents.view(mask_latents.shape[0], -1)#.unsqueeze(1)
            ...
            with accelerator.accumulate(model):
                ...
                if np.random.random() < config['train']['conditioning_dropout']:
                    mask_embedding = torch.zeros_like(mask_embedding)
                
                # Predict the noise residual
                model_output = model(noisy_images,
                                     timesteps,
                                     encoder_hidden_states=None,
                                     added_cond_kwargs={'image_embeds': mask_embedding}).sample 
        ...
        if accelerator.is_main_process:
            ...
            if epoch % config['samples']['samples_freq'] == 0 or epoch == config['train']['epochs'] - 1:
                ...
                for g in range(nsamp):
                    ...
                    images = pipeline(
                        generator=generator,
                        image_cond_latent=reference_mask_reduced_latents.to('cuda'),
                        guidance_scale = 7.5,
                    ...
                    if image_key == 'image' and 'segmentation' not in config['model']:
                        ...
                        np.save(f"{BASE_DIR}/samples/samples_{global_step:06d}-{g:02d}.npy", images_chann)

                        images = AE.decode(images_chann.permute(0, 3, 1, 2).to('cuda'), return_dict=False)[0] 
                        images = (images / 2 + 0.5).clamp(0, 1)
                        images = images.permute(0, 2, 3, 1).float().numpy()
                        grid = T.ToPILImage(mode='RGB')(make_grid(images, nrow=1))
                        grid.save(f"{BASE_DIR}/samples/sample_{global_step:06d}_{g:02d}.jpg")

                        np.save(f"{BASE_DIR}/samples/masks_{epoch:04d}-{global_step:06d}-{g:02d}.npy", masks_chann)
    ...
    if accelerator.is_main_process:
        ...
        for g in range(config['samples']['samples_gen']):
            ...
            if image_key == 'image' and 'segmentation' not in config['model']:
                ...
                images = torch.from_numpy(images) 
                images = AE.decode(images.permute(0, 3, 1, 2).to('cuda'), return_dict=False)[0] 
                images = (images / 2 + 0.5).clamp(0, 1)
                images = images.permute(0, 2, 3, 1).float().numpy()
                grid = T.ToPILImage(mode='RGB')(make_grid(images, nrow=1))
                ...
                np.save(f"{BASE_DIR}/final/samples_{global_step:06d}-{g:02d}.npy", images_chann)
                ...
                np.save(f"{BASE_DIR}/final/masks_{epoch:04d}-{global_step:06d}-{g:02d}.npy", masks_chann)
