"""
Generation of images using conditional diffusion model where the conditioning is the gleason score. 
"""
import argparse
import inspect
import logging
...
from diffusion.models.unet_2d_condition import UNet2DConditionModel, UNet2DConditionOutput
...
def main(config):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                # Save the current step count or other state information
                with open(os.path.join(output_dir, "training_state.json"), "w") as f:
                    json.dump({"step": accelerator.step}, f)
        ...
        def load_model_hook(models, input_dir):
            ...
            # Load the saved step count or other state information
            state_path = os.path.join(input_dir, "training_state.json")
            if os.path.exists(state_path):
                with open(state_path, "r") as f:
                    state = json.load(f)
                    global_step = state.get("step", 0)
                    accelerator.step = global_step  # Assign the step to accelerator
                    print(f"Restored step: {global_step}")  # Debugging line
    ...
    accelerator.init_trackers(config['name'][14:], config=hps, 
                              init_kwargs={"wandb":
