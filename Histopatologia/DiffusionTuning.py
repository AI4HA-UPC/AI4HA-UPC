"""
Fine-tune conditional diffusion model using the diff-tuning approach. 
"""
import argparse
import inspect
import logging
...
from diffusers import DDPMScheduler, DDIMScheduler, DPMSolverMultistepScheduler, PNDMScheduler, EulerDiscreteScheduler
...
def ordinal_relationship_loss(predictions, classes):
    """
    Calculate the ordinal relationship loss for each batch element.
    Args:
    ...
    for i in range(batch_size):
        ...
        for j in range(batch_size):
            for k in range(batch_size):
                if i != j and j != k and classes[i] < classes[j] < classes[k]:
                    p, q, r = classes[i].item(), classes[j].item(), classes[k].item()
                    d_p_r = torch.norm(predictions[i] - predictions[k], p=2)
                    d_p_q = torch.norm(predictions[i] - predictions[j], p=2)
                    d_q_r = torch.norm(predictions[j] - predictions[k], p=2)
                    loss_sum += (d_p_r - (d_p_q + d_q_r)) ** 2
                    count += 1
        if count > 0:
            losses[i] = loss_sum / count
...
def main(config):
    ...
    #model = UNet2DConditionModel().from_pretrained(f'{config['model']['path']}/model.last/unet')

    AE = LATENTS[config['latents']['type']].from_pretrained(config['latents']['model']).eval()
    AE.to('cuda').requires_grad_(False)

    clip_model = CLIPTextModel.from_pretrained('/gpfs/projects/bsc70/MN4/bsc70/bsc70174/Models/stable-diffusion-xl-base-1.0', subfolder='text_encoder')
    tokenizer = AutoTokenizer.from_pretrained('/gpfs/projects/bsc70/MN4/bsc70/bsc70174/Models/stable-diffusion-xl-base-1.0/tokenizer')
    ...
    if config['model']['embeddings'] == 'encoder':
        ...
        for i in new_labels:
            if i > 2:
                prompt = f"Generate an histopathological image of Prostate Cancer of Gleason score {new_labels[i]}"
            else:
                prompt = f"Generate an histopathological image of Prostate Cancer of label {new_labels[i]}"
    ...
    gen_data = instantiate_from_config(config['dataset']['synthetic'])

    num_train_data = len(train_data)
    num_gen_data = len(gen_data)

    if num_train_data < num_gen_data:
        desired_gen_dataset_size = np.round(num_train_data)
        gen_random_indices = torch.randperm(len(gen_data))[:desired_gen_dataset_size]
        reduced_gen_dataset = torch.utils.data.Subset(gen_data, gen_random_indices)

        gen_dataloader = torch.utils.data.DataLoader(reduced_gen_dataset, **config['dataset']["dataloader"])
        train_dataloader = torch.utils.data.DataLoader(train_data, **config['dataset']["dataloader"])

    if num_train_data > num_gen_data:
        desired_train_dataset_size = np.round(num_gen_data)
        train_random_indices = torch.randperm(len(train_data))[:desired_train_dataset_size]
        reduced_train_dataset = torch.utils.data.Subset(train_data, train_random_indices)

        train_dataloader = torch.utils.data.DataLoader(reduced_train_dataset, **config['dataset']["dataloader"])
        gen_dataloader = torch.utils.data.DataLoader(gen_data, **config['dataset']["dataloader"])
    ...
    num_gen_data = len(gen_dataloader)
    logger.info(f"Train dataset size: {num_train_data}")
    logger.info(f"Synthetic dataset size: {num_gen_data}")
    ...
    logger.info(f"  Num real examples = {num_train_data}")
    logger.info(f"  Num synthetic examples = {num_gen_data}")
    ...
    accelerator.init_trackers(config['name'][-120:], config=hps, 
                              init_kwargs={"wandb":
    ...
    if 'zero_terminal_snr' in config['diffuser']:
        if config['diffuser']['zero_terminal_snr']:
            ...
            noise_scheduler.sigmas = ((1 - new_alphas_cumprod) / new_alphas_cumprod+0.0001) ** 0.5
    """
    train_loss = []
    train_adaptation_loss = []
    train_retention_loss = []

    adaptation_weights = torch.arange(0, noise_scheduler.config.num_train_timesteps, dtype=torch.float32)
    adaptation_weights = adaptation_weights / noise_scheduler.config.num_train_timesteps
    retention_weights = torch.arange(noise_scheduler.config.num_train_timesteps-1, 0, -1, dtype=torch.float32)
    retention_weights = retention_weights / noise_scheduler.config.num_train_timesteps

    print("Adaptation weights: ", adaptation_weights)
    print("Retention weights: ", retention_weights)
    ...
    for epoch in range(first_epoch, config['train']['epochs']):
        ...
        loss_epoch = 0
        loss_adaptation_epoch = 0
        loss_retention_epoch = 0

        progress_bar = tqdm(total=num_update_steps_per_epoch, disable=not accelerator.is_local_main_process)
        progress_bar.set_description(f"Epoch {epoch}")

        for step, (batch_gen, batch_train) in enumerate(zip(gen_dataloader, train_dataloader)):
            ...
            clean_images_latents = norm_tfm(batch_gen[image_key]).to('cuda')
            loss_retention_sum = 0

            if "segmentation" in config['model']:
                if config['model']['segmentation'] > 0:
                    clean_images_latents = torch.cat((clean_images_latents, batch_gen['segmentation']), 1)
            ...
            timesteps = torch.multinomial(
                retention_weights, bsz, replacement=True
            ).to(clean_images_latents.device).long()
            ...
            with accelerator.accumulate(model):
                ...
                labels = None if 'gleason' not in batch_gen else batch_gen['gleason']
                ...
                if config['diffuser']['prediction_type'] == "epsilon":
                    ...
                    loss_retention = loss_weight * F.mse_loss(model_output, noise)
                    loss_retention_sum += loss_retention
                    loss_retention_epoch += loss_retention.mean().detach().cpu()
                    ...
                    loss_retention = loss_weight * F.mse_loss(model_output, clean_images_latents, reduction="none") 
                    loss_retention_sum += loss_retention
                    loss_retention_epoch += loss_retention.mean().detach().cpu()
                    ...
                    loss_retention = loss_weight * F.mse_loss(model_output, v_pred, reduction="none") 
                    loss_retention_sum += loss_retention
                    loss_retention_epoch += loss_retention.mean().detach().cpu()
                else:
            ...
            clean_images_latents = norm_tfm(batch_train[image_key])
            loss_adaptation_sum = 0

            if "segmentation" in config['model']:
                if config['model']['segmentation'] > 0:
                    clean_images_latents = torch.cat((clean_images_latents, batch_train['segmentation']), 1)
            ...
            # Sample a random timestep for each image
            timesteps = torch.multinomial(
                adaptation_weights, bsz, replacement=True
            ).to(clean_images_latents.device).long()
            ...
            with accelerator.accumulate(model):
                # Predict the noise residual
                # labels = None if 'class_ohe' not in batch else torch.squeeze(batch['class_ohe'])
                labels = None if 'gleason' not in batch_train else batch_train['gleason']
                ...
                if config['diffuser']['prediction_type'] == "epsilon":
                    ...
                    loss_adaptation = loss_weight * F.mse_loss(model_output, noise)
                    loss_adaptation_sum += loss_adaptation
                    loss_adaptation_epoch += loss_adaptation.mean().detach().cpu()
                    ...
                    loss_adaptation = loss_weight * F.mse_loss(model_output, clean_images_latents, reduction="none") 
                    loss_adaptation_sum += loss_adaptation
                    loss_adaptation_epoch += loss_adaptation.mean().detach().cpu()
                    ...
                    loss_adaptation = loss_weight * F.mse_loss(model_output, v_pred, reduction="none") 
                    loss_adaptation_sum += loss_adaptation
                    loss_adaptation_epoch += loss_adaptation.mean().detach().cpu()
                else:
                    raise ValueError(f"Unsupported prediction type: {config['diffuser']['prediction_type']}")
            ...
            loss = loss_retention_sum + loss_adaptation_sum  
            loss = loss.mean()
            ...
            if accelerator.sync_gradients:
                ...
                logs = {"loss": loss_epoch.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "step": global_step}
                if 'ema' in config:
        ...
        train_adaptation_loss.append(loss_adaptation_epoch / len(train_dataloader))
        train_retention_loss.append(loss_retention_epoch / len(train_dataloader))
        train_loss.append(loss_epoch)
        print("Epoch: ", epoch)
        print("Adaptation loss: ", train_adaptation_loss)
        print("Retention loss: ", train_retention_loss)
        print("Train loss: ", train_loss)

        # Generate sample images for visual inspection
        if accelerator.is_main_process:
            accelerator.get_tracker("wandb").log({"loss": train_loss, "adaptation_loss": train_adaptation_loss, "retention_loss": train_retention_loss, "lr": lr_scheduler.get_last_lr()[0]},step=global_step)
    ...
    train_retention_loss_floats = [float(tensor) for tensor in train_retention_loss]
    train_adaptation_loss_floats = [float(tensor) for tensor in train_adaptation_loss]

    df = pd.DataFrame({
        'train_loss': train_loss_floats,
        'train_retention_loss': train_retention_loss_floats,
        'train_adaptation_loss': train_adaptation_loss_floats
    })
    ...
    plt.plot(list(range(first_epoch, epoch+1)), train_retention_loss, 'ro-', label='Retention loss')
    plt.plot(list(range(first_epoch, epoch+1)), train_adaptation_loss, 'go-', label='Adaptation loss')
    plt.title('Training Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'{BASE_DIR}/train_loss.png')
