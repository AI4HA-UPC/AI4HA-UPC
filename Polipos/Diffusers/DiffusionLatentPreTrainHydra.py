...
def main(config):
    ...
    if path is None and BOOT_DIR is not None:
        ...
        if boot_path is not None:
            ...
            model = UNet2DModel.from_pretrained(
                os.path.join(f'{BOOT_DIR}/checkpoints/', boot_path),
                subfolder="unet")
    ...
    if 'std_scaling' in config['train']:
        ...
        if config['train']['std_scaling'] == "full":
            xmean, xstd = dataset_statistics(train_dataloader,
                                             image_key,
                                             channels=in_channels)
            norm_tfm = T.Normalize(xmean, xstd)
        elif config['train']['std_scaling'] == "batch":
            xstd = dataset_first_batch_std(train_data,
    ...
    for epoch in range(first_epoch, config['train']['num_epochs']):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            noisy_images = noise_scheduler.add_noise(clean_images_latents,
                                                     noise, timesteps)
            ...
            with accelerator.accumulate(model):
                ...
                if 'weighting' in config['diffuser']:
                    ...
                    e_weights = loss_weighting(
                        ...
                        config['diffuser']['weighting']['type'], snr_weights,
                        timesteps)
                else:
                ...
                if config['diffuser']['prediction_type'] == "epsilon":
                    ...
                    loss = e_weights * F.mse_loss(
                        model_output, clean_images_latents, reduction="none")
                    loss = loss.mean()
                else:
                    raise ValueError(
