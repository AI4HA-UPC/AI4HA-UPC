...
from ai4ha.layers import Patchify, MixPatchify
# from ai4ha.util import tof
...
def extra_masked_autoencoder_exp_name(config):
    ...
    name = "ExtraMAE"
    name += f"_le{config['model']['layers_encoder']}"
    name += f"_ld{config['model']['layers_decoder']}"
    name += f"_he{config['model']['heads_encoder']}"
    name += f"_hd{config['model']['heads_decoder']}"
    ...
    if 'mask_loss' in config['model']:
        name += f"_ml{tof(config['model']['mask_loss'])}"
    else:
        name += "_mlT"
    if 'norm_layer' in config['model']:
        name += f"_nl{tof(config['model']['norm_layer'])}"
    else:
        name += "_nlT"
    if 'patcher' in config['model']:
        name += f"_p{config['model']['patcher']}"
    else:
        name += "_pnormal"
    return name
...
class ExtraMAEEncoder(nn.Module):
    ...
    def __init__(
        ...
        if patcher == 'normal':
            self.patchify = Patchify(in_channels, embed_dimension,
                                     patch_size, norm_layer)
        elif patcher == 'mix':
            self.patchify = MixPatchify(in_channels, embed_dimension,
                                        patch_size, norm_layer)
        else:
            raise ValueError(f'Patcher {patcher} not implemented')
