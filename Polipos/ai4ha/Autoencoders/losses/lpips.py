...
class LPIPS(nn.Module):
    ...
    def __init__(self, root, use_dropout=True):
        super().__init__()
        ...
        self.load_from_pretrained(root)
        for param in self.parameters():
            param.requires_grad = False

    def load_from_pretrained(self, root, name="vgg_lpips"):
        # ckpt = os.path.join(root, "/pretrained/vgg.pth")
        ckpt = f"{root}/pretrained/vgg.pth"
        print(ckpt)
    ...
    def forward(self, input, target):
        ...
        for kk in range(len(self.chns)):
            ...
            feats0[kk], feats1[kk] = normalize(outs0[kk]), normalize(outs1[kk])
