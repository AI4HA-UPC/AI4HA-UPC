...
import numpy as np
from ai4ha.layers import WaveGANDeconvolution


class WaveGANDeconvGenerator(nn.Module):
    """_WaveGAN generator with deconvolutions/upsampling_

    Applies a succesion of deconvolutions/upsampling to the input latent vector to 
    ...
    def __init__(self,
                 ...
                 normalization=None,
                 upsampling=False):
        super(WaveGANDeconvGenerator, self).__init__()
        ...
        if kernel_size % 2 == 0 and upsampling:
        ...
        # Check if the number of deconvolutions is coherent with the initial
        #  number of channels
        ...
        if n_classes > 0:
            if class_embedding_dim > 0:
                self.class_embedding = nn.Embedding(n_classes,
                                                    class_embedding_dim)
        ...
        self.deconv_out = WaveGANDeconvolution(
            init_channels,
            output_channels,
            kernel_size,
            stride,
