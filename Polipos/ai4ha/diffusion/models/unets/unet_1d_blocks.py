...
class ValueFunctionMidBlock1D(nn.Module):
    def __init__(
        self, in_channels: int, out_channels: int, embed_dim: int, kernel_size: int = 5
    ):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        ...
        self.res1 = ResidualTemporalBlock1D(
            in_channels, in_channels // 2, embed_dim=embed_dim, kernel_size=kernel_size
        )
...
class MidResTemporalBlock1D(nn.Module):
    def __init__(
        ...
        resnets = [
            ResidualTemporalBlock1D(
                in_channels, out_channels, embed_dim=embed_dim, kernel_size=kernel_size
            )
        ]
